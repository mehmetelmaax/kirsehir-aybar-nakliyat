...
const reportOutputPathGeo = path.join(__dirname, '..', 'SEO-GEO-DURUM.md');
const reportOutputPathNormal = path.join(__dirname, '..', 'SEO-DURUM.md');
...
function countInternalLinks(html) {
  ...
  for (const match of linkMatches) {
    ...
    const isInternal = href.startsWith('/') || href.startsWith('https://kirsehiraybarnakliyat.com.tr');
...
function calculateGeoScore(html, wordCount) {
  ...
  if (firstParagraphMatch) {
    ...
    const hasBrand = text.includes('Kırşehir Aybar') || text.includes('Aybar Nakliyat');
...
function getKeywordsForPage(route) {
  if (route === '/') return 'kırşehir evden eve nakliyat, kırşehir nakliyat';
  if (route.includes('kaman')) return 'kaman evden eve nakliyat, kaman nakliyat';
  if (route.includes('mucur')) return 'mucur evden eve nakliyat, mucur nakliyat';
  if (route.includes('cicekdagi')) return 'çiçekdağı evden eve nakliyat';
  if (route.includes('akpinar')) return 'akpınar evden eve nakliyat';
  if (route.includes('boztepe')) return 'boztepe evden eve nakliyat';
  if (route.includes('akcakent')) return 'akçakent evden eve nakliyat';
  if (route.includes('asansorlu')) return 'kırşehir asansörlü nakliyat, kiralık asansör kırşehir';
  if (route.includes('fiyatlari')) return 'kırşehir nakliyat fiyatları, ev taşıma fiyatları kırşehir';
  if (route.includes('firmalari')) return 'kırşehir nakliyat firmaları, en iyi kırşehir nakliye';
  if (route.includes('sehirlerarasi')) return 'kırşehir şehirlerarası nakliyat, şehirler arası ev taşıma';
  return 'kırşehir nakliyat rehberleri';
}

function generateSeoReport() {
  console.log('Generating SEO & GEO Status Reports (SEO-DURUM.md & SEO-GEO-DURUM.md)...');
  ...
  // 1. Generate SEO-GEO-DURUM.md
  let mdGeo = `# SEO ve GEO Durum Raporu (SEO-GEO-DURUM.md)
  
*Bu rapor Next.js build çıktıları (.next/server/app) taranarak otomatik üretilmiştir.*
*Son Tarama Tarihi: ${todayStr}*
...
`;
  ...
  rows.forEach(r => {
    ...
    mdGeo += `| \`${r.route}\` | ${getPageType(r.route)} | ${r.title} | ${r.wordCount} | ${r.schemas.slice(0, 40)}... | ${status} | ${scoreStars} (${r.geoScore}/5) |\n`;
  });
...
`;
  ...
  rows.forEach(r => {
    if (r.indexable) {
      mdGeo += `| \`${getKeywordsForPage(r.route)}\` | [\`${r.route}\`](${r.route}) |\n`;
    }
  });
...
`;
  ...
  fs.writeFileSync(reportOutputPathGeo, mdGeo, 'utf8');
  console.log(`SEO-GEO-DURUM.md successfully written.`);

  // 2. Generate SEO-DURUM.md
  let mdNormal = `# SEO Durum Raporu (SEO-DURUM.md)

*Bu rapor Next.js build çıktıları (.next/server/app) taranarak otomatik üretilmiştir.*
*Son Güncelleme: ${todayStr}*
...
`;
  ...
  rows.forEach(r => {
    const canonical = `https://kirsehiraybarnakliyat.com.tr${r.route === '/' ? '' : r.route}`;
    mdNormal += `| \`${r.route}\` | ${r.title} | ${r.description} | \`${canonical}\` | ${r.schemas.slice(0, 50)}... | **${r.wordCount}** | **${r.internalLinks}** |\n`;
  });
...
`;
  ...
  fs.writeFileSync(reportOutputPathNormal, mdNormal, 'utf8');
  console.log(`SEO-DURUM.md successfully written.`);
}
