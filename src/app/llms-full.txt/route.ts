...
> ${SITE.name}, Kırşehir genelinde K3 yetki belgesiyle ${FACTS.foundedYear} yılından bu yana asansörlü ve sigortalı evden eve nakliyat hizmeti vermektedir.
...
`;
  ...
  const routesList = [
    ...
    { route: '/kirsehir-nakliyat-fiyatlari', name: 'Fiyat Rehberi', filePath: 'kirsehir-nakliyat-fiyatlari.html' },
    ...SERVICES.map(s => ({
  ...
  for (const item of routesList) {
    ...
    if (fs.existsSync(fullFilePath)) {
      ...
      pageText = `Aybar Nakliyat ${item.name} hizmet detayları. Sabit fiyat garantisiyle ${FACTS.insurer} sigortalı ve marangozlu evden eve nakliye çözümleri. Lütfen ${SITE.url}${item.route} adresini ziyaret edin.`;
    }
