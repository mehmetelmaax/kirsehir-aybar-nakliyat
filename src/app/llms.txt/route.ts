...
> ${SITE.name}, Kırşehir merkezli K3 yetki belgeli, asansörlü ve marangozlu evden eve nakliyat firmasıdır. Sabit fiyat garantisiyle ${FACTS.insurer} güvencesinde sigortalı eşya taşıma hizmeti vermektedir.
...
- Hizmet Kapsamı: ${FACTS.districtCount} Adet Kırşehir İlçesi ve 81 İle Şehirlerarası Nakliye
...
${Object.values(routesDatabase).map(r => `- [Kırşehir - ${r.city} Nakliyat](${SITE.url}/rotalar/${r.slug}): Mesafe yaklaşık ${r.distanceKm} km, ortalama seyahat süresi ${r.durationHours} saattir.`).join('\n')}

## Fiyat Bilgisi
Fiyatlandırma detayları ve anlık maliyet hesaplama robotu için [Kırşehir Evden Eve Nakliyat Fiyatları](${SITE.url}/kirsehir-nakliyat-fiyatlari) sayfamızı ziyaret edin. Sitedeki standart şehiriçi ev taşıma bütçesi ortalama ₺${FACTS.priceMin} ile ₺${FACTS.priceMax} aralığındadır.
