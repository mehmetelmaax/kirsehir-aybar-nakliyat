...
export default function PriceCalculator() {
  ...
  const getWhatsAppLink = () => {
    if (!estimate) return '';
    const text = `Merhaba, Kırşehir Aybar Nakliyat web sitesindeki Fiyat Hesaplayıcı ile tahmini bütçe çıkardım:
- Ev Boyutu: ${rooms} Daire
- Mesafe Grubu: ${distanceType === 'sehirici' ? 'Merkez / Kaman Şehiriçi' : distanceType === 'ilceler' ? 'İlçe Sınırları' : 'Şehirlerarası'}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-off-white border border-gray-light rounded-xl px-4 py-2.5 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-orange/30 focus:border-orange transition-all"
                >
                  <option value="sehirici">Kırşehir Şehiriçi (Merkez/Kaman)</option>
                  <option value="ilceler">Kırşehir İlçeler Arası (Akpınar/Boztepe vb.)</option>
                  <option value="sehirlerarasi">Şehirlerarası (Kırşehir Dışı 81 İl)</option>
                </select>
              </div>
            </div>
