...
const services: ServiceItem[] = [
  {
    ...
    outcomeLine: 'Kırşehir merkez ilçeleri arasında aynı gün içinde sorunsuz ev teslimi.',
    mechanics: [
      'Merkez-Kaman hızlı servis',
  ...
  },
  {
    ...
    outcomeLine: "Kırşehir'den Türkiye'nin tüm illerine sigortalı ve zamanında teslimat.",
    mechanics: [
  ...
  },
  {
    ...
    bestFor: 'İş kesintisi yaşamak istemeyen Kırşehir esnafı ve şirketleri.',
...
export default function ServicesGrid() {
  return (
    <section className="py-24 bg-brand-light" id="hizmetler">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto text-center mb-16 space-y-4">
          ...
          <h2 className="font-display font-extrabold text-brand-dark text-3xl md:text-4xl lg:text-5xl tracking-tight leading-tight">
            Profesyonel Taşımacılık Hizmetleri
          </h2>
          <p className="text-brand-gray text-base md:text-lg leading-relaxed">
            {new Date().getFullYear() - FACTS.foundedYear} yıllık Kırşehir deneyimimizle sunduğumuz, tüm lojistik ihtiyaçlarınızı karşılayan odaklanmış çözümlerimiz.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-24 bg-brand-dark rounded-2xl shadow-xl p-6 border border-brand-accent/20 text-white">
            <div className="mb-4">
              <span className="text-brand-accent font-bold text-[10px] tracking-widest block">ÜCRETSİZ EKSPERTİZ</span>
              <h3 className="font-display font-black text-white text-xl mt-0.5">Taşınma Maliyeti Al</h3>
            </div>
          ...
          <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            {services.map((service) => {
              ...
              return (
                <div 
                  key={service.id}
                  className="bg-white rounded-2xl p-6 md:p-8 border border-slate-200/60 hover:border-brand-accent/30 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between group relative overflow-hidden cursor-pointer"
                >
                  <div className="absolute top-0 right-0 w-24 h-24 bg-slate-100 rounded-bl-full transition-colors group-hover:bg-brand-accent/5"></div>
                  
                  <div>
                    {/* Card Header (Icon & Names) */}
                    <div className="flex gap-4 items-center mb-6">
                      <div className="bg-brand-dark/5 text-brand-dark p-3 rounded-lg group-hover:bg-brand-accent group-hover:text-brand-dark transition-colors duration-300">
                        <Icon className="w-6 h-6" />
                      </div>
                      <div>
                        <h3 className="font-display font-bold text-brand-dark text-sm group-hover:text-brand-accent transition-colors leading-tight">
                          {service.name}
                        </h3>
                        <span className="text-slate-400 text-[10px] font-semibold tracking-wider block mt-1">
                    ...
                    {/* Outcome line */}
                    <p className="text-brand-dark font-bold text-xs md:text-sm leading-relaxed mb-6">
                      {service.outcomeLine}
                    </p>

                    {/* Mechanics (Bullets) */}
                    <ul className="space-y-3 mb-6 border-t border-b border-slate-100 py-4">
                      {service.mechanics.map((bullet, idx) => (
                        <li key={idx} className="flex items-center gap-2 text-xs text-brand-dark/85 font-semibold">
                    ...
                    <div className="space-y-2 mb-8 text-[11px]">
                      <div>
                        <span className="text-slate-400 font-bold">Önerilen Durum: </span>
                        <span className="text-brand-dark/70 font-semibold">{service.bestFor}</span>
                      </div>
                      <div>
                        <span className="text-slate-400 font-bold">Fiyatlama: </span>
                        <span className="text-brand-dark/70 font-semibold">{service.pricePosture}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <Link
                    href={service.route}
                    className="bg-brand-dark hover:bg-brand-accent text-white hover:text-brand-dark font-bold py-2.5 px-4 rounded transition-all duration-200 text-center text-xs flex items-center justify-center gap-2 group-hover:shadow-md active:scale-98"
                  >
