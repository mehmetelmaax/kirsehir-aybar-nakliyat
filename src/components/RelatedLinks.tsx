...
export default function RelatedLinks({ currentSlug, type, title }: RelatedLinksProps) {
  ...
  const getServiceAnchor = (name: string, index: number, prefix: string = '') => {
    const templates = [
      ...
      `Kırşehir ${prefix ? prefix + ' ' : ''}${name} çözümleri`,
  ...
  if (type === 'hizmet') {
    ...
    // 3 neighbors + 6 services + /kirsehir-nakliyat-fiyatlari
    ...
    // Add fiyatlari link
    links.push({
      href: '/kirsehir-nakliyat-fiyatlari',
      text: `${currentDistrict ? currentDistrict.name : 'Kırşehir'} ev taşıma fiyatları listesi`,
    });
  } 
