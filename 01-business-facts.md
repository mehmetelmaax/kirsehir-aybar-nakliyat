...
## Identity
- **Exact Trade Name on Site**: `KIRŞEHİR AYBAR NAKLİYAT` (appears on main page and profile page) and `KIRŞEHİR AYBAR EVDEN EVE NAKLİYAT` (appears in the footer copyright).
- **Alternate Names / "Aybar Evden Eve Nakliyat"**: "Aybar" is the main word used in title tags (`<title>AYBAR</title>`). "Aybar Evden Eve Nakliyat" appears in the footer copyright text combined as "KIRŞEHİR AYBAR EVDEN EVE NAKLİYAT".
...
- **Full Address**: Not stated on the website.
  - *Web search finding*: `Fatih Mahallesi, 73258 Sokak, No:9/1, Merkez/Kırşehir` [DOĞRULA: Bu adresi Google Business Profile ve resmi evraklarla teyit edin]
- **Phone Numbers**: `+90 537 312 34 47` (Found in top header menu, footer boxes, and overlay call images `img/erdem1.png`)
- **WhatsApp Number**: `+90 537 312 34 47` (Found in footer boxes and overlay call image `img/erdem.png` with link `https://api.whatsapp.com/send?phone=905373123447`)
...
## Operations
- **Services Offered**:
  - *Şehiriçi Evden Eve Nakliyat*: *"Şehir içi taşımacılık hizmetimizde, Kırşehir şehir merkezi ve ilçerine evden eve taşımacılık hizmeti veriyoruz."*
  ...
  - *İşyeri ve Ofis Nakliyatı* (Site uses: `Kırşehir İş Yeri Taşımacılığı` / `İşyeri ve Ofis Nakliyatı`)
  - *Profesyonel Eşya Paketleme* (Site uses: `Kırşehir Profesyonel Eşya Paketleme` / `Profesyonel Eşya Paketleme`)
  - *Ücretsiz Ekspertiz* (Site uses: `Kırşehir Eşya Ekspertiz Hizmeti` / `Ücretsiz Ekspertiz`)
- **Districts / Cities Claimed**: 
  - *"Kırşehir şehir merkezi ve ilçerine"* (Merkez and Akçakent explicitly appear in hidden crawler footer links)
...
| :--- | :--- | :--- |
| **Trade Name**: Kırşehir Aybar Nakliyat | `14.htm` (Firma Profili) | HIGH |
| **Phone**: +90 537 312 34 47 | `index.htm` | HIGH |
| **Email**: info@kirsehiraybarnakliyat.com.tr | `index.htm` | HIGH |
| **WhatsApp**: +90 537 312 34 47 | `index.htm` | HIGH |
| **Experience**: 20 Years | `index.htm` | HIGH |
| **Office Address**: Fatih Mh. 73258 Sk. No:9/1 Merkez/Kırşehir | External search (GBP / Maps) | MEDIUM (Requires verification) |
...
The domain is **`kirsehiraybarnakliyat.com.tr`** (exact keyword matching), while the branding inside the page is **`Kırşehir Aybar Nakliyat`** (sometimes written as just **`Aybar`**).

- **Brand Name 1: Kırşehir Evden Eve Asansörlü Nakliyat (Domain-exact)**
  - *Equity*: Matches exact local search intent in Kırşehir, but does not read as a real brand name. High spam perception.
- **Brand Name 2: Kırşehir Aybar Nakliyat (Actual Brand)**
  - *Equity*: Real business identity, matches Yandex/Google search listings and phone registration data.
- **Recommendation**:
  - Use **Kırşehir Aybar Nakliyat** as the primary visual brand name in headers, copy, and footer. This maintains high trust and legal safety.
  - Optimize semantic headers and meta descriptions to target "Kırşehir Evden Eve Nakliyat" and "Kırşehir Asansörlü Nakliyat" to leverage the domain's keyword-matching value without looking generic or untrustworthy to human visitors.
